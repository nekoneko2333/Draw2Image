...
# 核心修复：强制开启 V1 兼容模式并禁用立即执行
tf.compat.v1.disable_eager_execution()
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'

# --- 3. 重新定义 Keras 接口 (绕过 Keras 3.0 的路径限制) ---
# 核心修复：使用 tf_keras 避开 Keras 3 的严格检查
try:
    import tf_keras as keras
    from tf_keras import backend as K
    from tf_keras.models import Model
    from tf_keras.layers import Input, Conv2D, AveragePooling2D, MaxPooling2D
    from tf_keras.utils import get_file
except ImportError:
    # 如果没装 tf-keras，回退到 TF 内置兼容层
    ...
    AveragePooling2D = tf.compat.v1.keras.layers.AveragePooling2D
    MaxPooling2D = tf.compat.v1.keras.layers.MaxPooling2D
    get_file = tf.compat.v1.keras.utils.get_file

# 兼容脚本后续使用的旧版类名
Convolution2D = Conv2D

# 尝试定义转换函数，如果新版没有则定义为空操作
...
try:
    ...
    print(f"❌ 权重加载失败，请检查网络或手动下载权重到 ~/.keras/models: {e}")
    exit()
...
for i in range(num_iter):
    ...
    x, min_val, info = fmin_l_bfgs_b(evaluator.loss, x.flatten(), fprime=evaluator.grads, maxfun=7, maxiter=1)
