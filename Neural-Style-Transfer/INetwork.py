...
try:
    ...
    try:
        from tf_keras.utils.layer_utils import convert_all_kernels_in_model
    except ImportError:
        convert_all_kernels_in_model = lambda model: None  # Keras 3 不再需要
    print("[INetwork] 使用 tf_keras 后端")
except ImportError:
    # 回退到 tf.compat.v1.keras
    tf_keras = None
    from tensorflow.compat.v1.keras.models import Model
    from tensorflow.compat.v1.keras.layers import InputLayer, Conv2D, AveragePooling2D, MaxPooling2D
    from tensorflow.compat.v1.keras import backend as K
    from tensorflow.compat.v1.keras.utils import get_file
    try:
        from tensorflow.compat.v1.keras.utils.layer_utils import convert_all_kernels_in_model
    except ImportError:
        convert_all_kernels_in_model = lambda model: None  # TF 2.x 不再需要
    print("[INetwork] 使用 tensorflow.compat.v1.keras 后端")
...
if K.image_data_format() == "channels_first":
    combination_image = tf.compat.v1.placeholder(tf.float32, shape=(1, 3, img_width, img_height))
else:
    combination_image = tf.compat.v1.placeholder(tf.float32, shape=(1, img_width, img_height, 3))
...
# 使用 InputLayer 替代 Input(tensor=...) 以兼容 Keras 3
ip = InputLayer(input_tensor=input_tensor, batch_input_shape=shape).output
